...
function AddExcersiseScreen(props) {
    const exrname=props.route.params.exname
    ...
    const addexerciseplan=async()=>{
        ...
        try
        {
            ...
            const docRef = await addDoc(collection(db, "exerciseschedule"), {
                exercise: exrname,
                workoutplan: sp[0].data.name,
                duration: sp[0].data.duration,
                exercisetime:sp[0].data.workouttime,
